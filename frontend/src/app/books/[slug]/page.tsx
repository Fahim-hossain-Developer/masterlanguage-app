...
export default function BookReaderPage({
  ...
  const [saved, setSaved] = useState(() =>
    book ? isItemBookmarked(book.id) : false
  );
  ...
  if (!book) {
    return (
      <div className="min-h-screen flex flex-col bg-slate-50 text-slate-900">
        <MainNavbar />
        <main className="flex-1 mx-auto max-w-4xl px-4 py-16 text-center">
          <p className="text-sm font-bold text-slate-700">Book not found.</p>
          <Link
            href="/books"
      ...
      <main className="flex-1 mx-auto w-full max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        >
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-slate-900 px-4 py-3 text-white">
            {/* Mode Switcher or Page Navigation */}
            <div className="flex items-center gap-2">
              {hasUploadedPdf && (
                <div className="flex items-center rounded-lg bg-slate-800 p-0.5 mr-2">
                  <button
                    type="button"
              ...
              {(!hasUploadedPdf || viewMode === "text") && (
                <>
                  ...
                  <span className="text-xs font-bold">
                    Page {currentPage} of {Math.max(1, book.chapters.length)}
                  </span>
                  <button
                    type="button"
                    ...
                    onClick={() =>
                      setCurrentPage((p) =>
                        Math.min(book.chapters.length, p + 1)
                      )
                    }
                    className="rounded-lg bg-slate-800 p-1.5 hover:bg-slate-700 disabled:opacity-40 cursor-pointer"
                  >
            ...
            {/* In-Book Search (for Text Reader) */}
            {(!hasUploadedPdf || viewMode === "text") && (
